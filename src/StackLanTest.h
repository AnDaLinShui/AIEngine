...
struct StackLanTest: public StackTest {
	//Protocols
	...
	SharedPointer<FlowForwarder> ff_pop;
		...
		imap->setFlowManager(flow_table_tcp);	
		pop->setFlowManager(flow_table_tcp);	
	...
	void setDynamicAllocatedMemory(bool value) {
		...
		udp->setDynamicAllocatedMemory(value);
	}
	...
	void statistics() {
		...
		std::cout << std::endl;
		icmp->statistics(std::cout, 5);
		std::cout << std::endl;
		http->statistics(std::cout, 5);
		std::cout << std::endl;
	...
	void dumpFlows() {
		...
		flow_table_tcp->showFlows(std::cout,100);
		flow_table_udp->showFlows(std::cout,100);
	}
