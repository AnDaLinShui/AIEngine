...
FlowForwarder::FlowForwarder():
	...
	muxDown_(),
	protocol_id_(0),
	flowForwarderVector_(),
	flow_func_([&] (Flow*) { return; }),
	check_func_([&] (Packet&) { return true; })
	{} 

void FlowForwarder::insertUpFlowForwarder(WeakPointer<FlowForwarder> ff) { 

	flowForwarderVector_.insert(flowForwarderVector_.begin(), ff);
}

void FlowForwarder::addUpFlowForwarder(WeakPointer<FlowForwarder> ff) { 

	flowForwarderVector_.push_back(ff); 
}

void FlowForwarder::removeUpFlowForwarder() { 

	flowForwarderVector_.pop_back(); 
}

void FlowForwarder::removeUpFlowForwarder(WeakPointer<FlowForwarder> ff) {

	auto it = std::find_if(flowForwarderVector_.begin(),flowForwarderVector_.end(),
		[&] (WeakPointer<FlowForwarder> &p) {
			return p.lock() == ff.lock();
		}
	); 
