...
BOOST_PYTHON_MODULE(pyaiengine)
{
	...
	void (NetworkStack::*showNetFlows1)() = 					&NetworkStack::showFlows;
	void (NetworkStack::*showNetFlows2)(const std::string&) = 			&NetworkStack::showFlows;
	void (NetworkStack::*showNetFlowsLimit1)(int) = 				&NetworkStack::showFlows;
	void (NetworkStack::*showNetFlowsLimit2)(const std::string&, int) = 		&NetworkStack::showFlows;
		...
		.def("show_flows", showNetFlows1,
			help_show_flows)
		.def("show_flows", showNetFlows2,
			help_show_flows)
		.def("show_flows", showNetFlowsLimit1,
			help_show_flows)
	...
	;
        ...
        boost::python::class_<StackMobileIPv6, bases<NetworkStack> >("StackMobileIPv6",
                ...
                .def_readonly("name", &StackMobileIPv6::getName,
                        help_stack_name)
                ...
                .add_property("udp_flow_manager", make_function(&StackMobileIPv6::getUDPFlowManager, return_internal_reference<>()),
                        help_stack_udp_flow_manager)
                .add_property("enable_frequency_engine", &StackMobileIPv6::isEnableFrequencyEngine, &StackMobileIPv6::enableFrequencyEngine,
                        help_enable_freq_engine)
                .add_property("enable_nids_engine", &StackMobileIPv6::isEnableNIDSEngine, &StackMobileIPv6::enableNIDSEngine,
                        help_enable_nids_engine)
	...
	void (PacketDispatcher::*showCurrentPayloadPacketPdis)() =		&PacketDispatcher::showCurrentPayloadPacket;
	...
	boost::python::class_<PacketDispatcher, boost::noncopyable>("PacketDispatcher",
		...
		.add_property("port", &PacketDispatcher::getPort, &PacketDispatcher::setPort,
			help_pdis_port)
	...
	;
        ...
        void (FlowManager::*showFlows)()				= &FlowManager::showFlows;
        void (FlowManager::*showFlows2)(int)				= &FlowManager::showFlows;
        void (FlowManager::*showFlowsByProtocol)(const std::string&)	= &FlowManager::showFlows;
        void (FlowManager::*showFlowsByProtocol2)(const std::string&, int)	= &FlowManager::showFlows;
	...
	boost::python::class_<FlowManager, SharedPointer<FlowManager>, boost::noncopyable >("FlowManager")
		...
		.def("show", showFlowsByProtocol2,
			help_flow_manager_show_flows_pr)
		.def("show", showFlows,
			help_flow_manager_show_flows)
		.def("show", showFlowsByProtocol,
			help_flow_manager_show_flows_pr)
		.def("flush", &FlowManager::flush,
	...
	boost::python::class_<DNSInfo, SharedPointer<DNSInfo>, boost::noncopyable>("DNSInfo")
		.def("__iter__", boost::python::range(&DNSInfo::begin, &DNSInfo::end),
			help_dns_info_iter)
		.add_property("query_type", &DNSInfo::getQueryType,
	...
	;
        ...
        boost::python::class_<SSLInfo, SharedPointer<SSLInfo>, boost::noncopyable>("SSLInfo")
                ...
                .add_property("issuer_name", &SSLInfo::getIssuerName,
                        help_ssl_info_issuer_name)
                .add_property("cipher", &SSLInfo::getCipher,
	...
	;
        ...
        ;
       	...
       	boost::python::class_<IMAPInfo, SharedPointer<IMAPInfo>, boost::noncopyable>("IMAPInfo")
                .add_property("user_name", &IMAPInfo::getUserName,
	...
	boost::python::class_<Frequencies, SharedPointer<Frequencies>, boost::noncopyable>("Frequencies")
		...
		.add_property("entropy", &Frequencies::getEntropy,
	...
	boost::python::class_<IPSet, bases<IPAbstractSet>, SharedPointer<IPSet>>("IPSet")
		...
		.def(init<const std::string&, boost::python::list&, boost::python::object>())
                .add_property("name", &IPSet::getName, &IPSet::getName,
			help_ip_set_name)
	...
	void (IPRadixTree::*showRadixIPAddresses)() 			=	&IPRadixTree::show;
		...
		.def(init<boost::python::list&>())
		.def(init<const std::string&, boost::python::list&>())
                .add_property("name", &IPRadixTree::getName, &IPRadixTree::setName,
                        help_iprad_set_name)
                .add_property("lookups", &IPRadixTree::getTotalLookups,
                        help_iprad_set_lookups)
                .add_property("lookups_in", &IPRadixTree::getTotalLookupsIn,
                        help_iprad_set_lookups_in)
                .add_property("lookups_out", &IPRadixTree::getTotalLookupsOut,
