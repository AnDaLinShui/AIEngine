...
log4cxx::LoggerPtr StackMobileIPv6::logger(log4cxx::Logger::getLogger("aiengine.stackmobile6"));
#endif

StackMobileIPv6::StackMobileIPv6(): 
	/* LCOV_EXCL_START Looks that coverage dont like this */
        ip_low_(IPProtocolPtr(new IPProtocol())),
        ip6_high_(IPv6ProtocolPtr(new IPv6Protocol())),
	...
	// Multiplexers
        mux_ip6_high_(MultiplexerPtr(new Multiplexer())),
        ...
        mux_icmp6_(MultiplexerPtr(new Multiplexer())),
	...
	setName("MobileIPv6 Network Stack");
	...
	ff_gprs_->addFlowFunction(std::bind(&GPRSProtocol::processFlow, gprs_, std::placeholders::_1));
     	...
     	// configure the high IPv6 handler
        ip6_high_->setMultiplexer(mux_ip6_high_);
        mux_ip6_high_->setProtocol(static_cast<ProtocolPtr>(ip6_high_));
        mux_ip6_high_->setProtocolIdentifier(ETHERTYPE_IPV6);
        mux_ip6_high_->setHeaderSize(ip6_high_->getHeaderSize());
        mux_ip6_high_->addChecker(std::bind(&IPv6Protocol::ip6Checker, ip6_high_, std::placeholders::_1));
        mux_ip6_high_->addPacketFunction(std::bind(&IPv6Protocol::processPacket, ip6_high_, std::placeholders::_1));
	...
	mux_tcp_->addPacketFunction(std::bind(&TCPProtocol::processPacket, tcp_, std::placeholders::_1));
        ...
        icmp6_->setMultiplexer(mux_icmp6_);
        mux_icmp6_->setProtocol(static_cast<ProtocolPtr>(icmp6_));
        mux_icmp6_->setProtocolIdentifier(IPPROTO_ICMPV6);
        mux_icmp6_->setHeaderSize(icmp6_->getHeaderSize());
        mux_icmp6_->addChecker(std::bind(&ICMPv6Protocol::icmp6Checker, icmp6_, std::placeholders::_1));
	mux_icmp6_->addPacketFunction(std::bind(&ICMPv6Protocol::processPacket, icmp6_, std::placeholders::_1));
	...
	mux_gprs_->addUpMultiplexer(mux_ip6_high_, ETHERTYPE_IPV6);
        mux_ip6_high_->addDownMultiplexer(mux_gprs_);
        mux_ip6_high_->addUpMultiplexer(mux_icmp6_, IPPROTO_ICMPV6);
	mux_icmp6_->addDownMultiplexer(mux_ip6_high_);
	mux_ip6_high_->addUpMultiplexer(mux_tcp_, IPPROTO_TCP);
	mux_tcp_->addDownMultiplexer(mux_ip6_high_);
	mux_ip6_high_->addUpMultiplexer(mux_udp_high_, IPPROTO_UDP);
	mux_udp_high_->addDownMultiplexer(mux_ip6_high_);
	...
	// Tell to the GPRS layer that IPv6 packets will arrive
	gprs_->setIPProtocolType(ETHERTYPE_IPV6);
        ...
        ip6_high_->setAnomalyManager(anomaly_);
...
void StackMobileIPv6::showFlows(std::basic_ostream<char> &out, const std::string &protoname, int limit) {
...
void StackMobileIPv6::showFlows(std::basic_ostream<char> &out, int limit) {
...
void StackMobileIPv6::statistics(std::basic_ostream<char> &out) const {

        super_::statistics(out);
}

void StackMobileIPv6::setTotalTCPFlows(int value) {
...
void StackMobileIPv6::setTotalUDPFlows(int value) {
...
int StackMobileIPv6::getTotalTCPFlows() const { return flow_cache_tcp_->getTotalFlows(); }

int StackMobileIPv6::getTotalUDPFlows() const { return flow_cache_udp_high_->getTotalFlows(); }

void StackMobileIPv6::enableFrequencyEngine(bool enable) {
...
void StackMobileIPv6::enableNIDSEngine(bool enable) {
...
void StackMobileIPv6::setFlowsTimeout(int timeout) {

        flow_table_tcp_->setTimeout(timeout);
        flow_table_udp_high_->setTimeout(timeout);
}

void StackMobileIPv6::setTCPRegexManager(const SharedPointer<RegexManager> &rm) {
...
void StackMobileIPv6::setUDPRegexManager(const SharedPointer<RegexManager> &rm) {
...
void StackMobileIPv6::setTCPIPSetManager(const SharedPointer<IPSetManager> &ipset_mng) {

        tcp_->setIPSetManager(ipset_mng);
        super_::setTCPIPSetManager(ipset_mng);
}

void StackMobileIPv6::setUDPIPSetManager(const SharedPointer<IPSetManager> &ipset_mng) {
...
void StackMobileIPv6::setTCPRegexManager(RegexManager *sig) {
...
void StackMobileIPv6::setUDPRegexManager(RegexManager *sig) {
...
void StackMobileIPv6::setTCPIPSetManager(IPSetManager *ipset_mng) {
...
void StackMobileIPv6::setUDPIPSetManager(IPSetManager *ipset_mng) {
...
std::tuple<Flow*, Flow*> StackMobileIPv6::getCurrentFlows() const {

        Flow *low_flow = udp_low_->getCurrentFlow();
        Flow *high_flow = nullptr;
	uint16_t proto = ip6_high_->getProtocol();
