...
int32_t IPBloomSet::getTotalBytes() const {

	return 0; // TODO bloom_.size();
}

void IPBloomSet::resetStatistics() {

        total_ips_not_on_set_ = 0;
        total_ips_on_set_ = 0;
}

void IPBloomSet::clear() {

	bloom_ = boost::bloom_filters::dynamic_bloom_filter<std::string>();
...
bool IPBloomSet::lookupIPAddress(const IPAddress &address) {

	return lookupIPAddress(std::string(address.getDstAddrDotNotation()));
}
