...
BOOST_AUTO_TEST_SUITE (usercache_static) 

BOOST_AUTO_TEST_CASE (test01)
{
	auto uc(new Cache<User>);
...
BOOST_AUTO_TEST_SUITE (usercache_dynamic) 

BOOST_AUTO_TEST_CASE (test01)
{
	auto uc(new Cache<User>);
    
	uc->setDynamicAllocatedMemory(true);
	...
	BOOST_CHECK(uc->getTotalAcquires() == 2);
        BOOST_CHECK(uc->getTotalFails() == 0);
        BOOST_CHECK(u2 != nullptr);
